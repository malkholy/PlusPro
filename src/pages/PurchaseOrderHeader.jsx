    ...
    <span>
      {parts.map((part, i) => 
        part.toLowerCase() === highlight.toLowerCase() 
          ? <mark key={i} style={{ backgroundColor: 'rgba(249,115,22,0.25)', color: 'var(--orange)', padding: '1px 2px', borderRadius: 2, fontWeight: 700 }}>{part}</mark> 
          : part
      )}
    </span>
  );
        ...
        <div style={{
          ...
          <div style={{ overflowY: 'auto', flex: 1 }}>
            ...
            {filteredOptions.length === 0 ? (
              <div style={{ padding: '10px', fontSize: 12, color: 'var(--muted)', textAlign: 'center' }}>No results found</div>
            ) : (
              filteredOptions.map(opt => (
                <div 
                  key={opt.VendorNumber}
                  onClick={() => {
                  ...
                  style={{
                    ...
                    cursor: 'pointer',
                    fontSize: 12,
                    background: value === opt.VendorNumber ? 'rgba(249,115,22,0.1)' : 'none',
                    color: value === opt.VendorNumber ? 'var(--orange)' : 'var(--text)',
                    borderBottom: '1px solid rgba(0,0,0,0.02)',
                    transition: 'all 0.1s'
                  }}
                  onMouseEnter={e => e.currentTarget.style.background = 'rgba(255,255,255,0.05)'}
                  onMouseLeave={e => e.currentTarget.style.background = value === opt.VendorNumber ? 'rgba(249,115,22,0.1)' : 'none'}
                >
                  <div style={{ fontWeight: 600, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{opt.VendorName}</div>
                  <div style={{ fontSize: 10, color: 'var(--muted)' }}>Code: {opt.VendorNumber}</div>
                </div>
              ))
            )}
          </div>
...
export default function PurchaseOrderHeaderPage({ user }) {
  const cache = window.__poHeaderCache = window.__poHeaderCache || {};
  ...
  const columns = [
    ...
    { 
      key: 'OrderStateDescription', 
      label: 'Status',
      render: (val) => {
        const status = (val || 'Draft').trim();
        ...
        return (
          ...
          >
            {highlightText(status, searchQuery)}
          </span>
        );
      }
    },
    ...
    { 
      ...
      label: 'Total Amount', 
      numeric: true, 
      render: (val, row) => formatAmount(val, row.OrderCurrency) 
    },
  ...
  const avgReceivedPct = totalCount > 0 
    ? (rows.reduce((sum, r) => {
        ...
        return sum + ((Number(r.TotalRecievedAmount || 0) / total) * 100);
      }, 0) / totalCount)
    : 0;

  const avgPaidPct = totalCount > 0 
    ? (rows.reduce((sum, r) => {
        const total = Number(r.TotalAmount || 0);
        if (total === 0) return sum + 100;
        return sum + ((Number(r.TotalPaidAmount || 0) / total) * 100);
      }, 0) / totalCount)
    : 0;

  // Custom KPI control panel to show above the grid
  const controlPanel = (
    <div style={{ 
      display: 'grid', 
      ...
      <div style={{ background: 'var(--surface)', padding: 16, border: '1px solid var(--border)', borderRadius: 14 }}>
        <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '.05em', marginBottom: 6 }}>Total Purchase Orders</div>
      ...
      <div style={{ background: 'var(--surface)', padding: 16, border: '1px solid var(--border)', borderRadius: 14 }}>
        ...
        <div style={{ fontSize: 24, fontWeight: 800, color: 'var(--green)' }}>{avgReceivedPct.toFixed(1)}%</div>
        <div style={{ display: 'flex', gap: 6, fontSize: 11, color: 'var(--muted)', marginTop: 4 }}>
          <div style={{ flex: 1, height: 4, background: 'var(--border)', borderRadius: 99, overflow: 'hidden', marginTop: 4 }}>
            <div style={{ width: `${avgReceivedPct}%`, height: '100%', background: 'var(--green)' }}></div>
          </div>
        </div>
      </div>
      <div style={{ background: 'var(--surface)', padding: 16, border: '1px solid var(--border)', borderRadius: 14 }}>
        <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '.05em', marginBottom: 6 }}>Payment Rate (Avg)</div>
        <div style={{ fontSize: 24, fontWeight: 800, color: 'var(--orange)' }}>{avgPaidPct.toFixed(1)}%</div>
        <div style={{ display: 'flex', gap: 6, fontSize: 11, color: 'var(--muted)', marginTop: 4 }}>
          <div style={{ flex: 1, height: 4, background: 'var(--border)', borderRadius: 99, overflow: 'hidden', marginTop: 4 }}>
            <div style={{ width: `${avgPaidPct}%`, height: '100%', background: 'var(--orange)' }}></div>
          </div>
        </div>
      </div>
    </div>
  ...
  const filterPanel = (
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap', flex: 1 }}>
        ...
        {/* Vendor */}
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, flex: 2, minWidth: 260, maxWidth: 420 }}>
  ...
  const renderDeliveryTab = () => {
    ...
    const fields = [
      ...
      { label: 'ETD', value: formatDate(firstLine.ETD) },
  ...
  const filteredRows = rows.filter(row => {
    ...
    return (
      ...
      String(row.OrderCreatedBy || '').toLowerCase().includes(q) ||
      String(row.OrderStateDescription || '').toLowerCase().includes(q)
    );
  });
  ...
  return (
    <div style={{ padding: '0 4px', display: 'flex', flexDirection: 'column', flex: 1, minHeight: 0 }}>
      ...
      <DataGrid
        ...
        subtitle="Track payment terms, delivery status, and invoice amounts across all active orders"
        columns={columns}
        rows={filteredRows}
        loading={loading}
        onRefresh={loadData}
      ...
      {isDrawerOpen && selectedPO && (
        <>
          ...
          >
            {/* Drawer Header */}
            {(() => {
              const currentIndex = filteredRows.findIndex(row => row.PurchaseOrderNumber === selectedPO.PurchaseOrderNumber);
                  ...
                  <div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      <span style={{ fontSize: 18, fontWeight: 800, color: 'var(--text)' }}>
                        PO #{selectedPO.PurchaseOrderNumber}
                      </span>
                      <span style={{
                        fontSize: 11,
                        fontWeight: 700,
            ...
            {/* Drawer Footer */}
            <div style={{ padding: '14px 20px', borderTop: '1px solid var(--border)', display: 'flex', justifyContent: 'flex-end', gap: 10, background: 'var(--soft)' }}>
              <button 
                onClick={() => setIsDrawerOpen(false)}
