...
export default function PlanningItemMaster({ user }) {
  ...
  const columns = [
    ...
    { key: 'DefaultMachineCode', label: 'Default Machine', width: 150, render: (val) => val || '—' },
    { key: 'DefaultFormulaCode', label: 'Default Formula', width: 150, render: (val) => val || '—' },
    { key: 'SaftyStock', label: 'Safety Stock', width: 110, align: 'right' },
    { key: 'LeadTime', label: 'Lead Time', width: 100, align: 'right' },
    { key: 'NetWeight', label: 'Net Weight (Kg)', width: 130, align: 'right', render: (val) => Number(val || 0).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 5 }) },
