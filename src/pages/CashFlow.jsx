...
const PANEL_GROUPS = [
  ...
  { title: 'Payables & Receivables', icon: '⚖️', accent: '#34d399', items: [
    ...
    [row => (Number(row.TotalDueCustomerInvoicesMonthEnding) || 0) - (Number(row.TotalDueVendorInvoicesMonthEnding) || 0), 'Diff (Customer - Vendor)', fmtDiff]
  ]},
...
export default function CashFlow({ user }) {
  ...
  return (
    <div style={{ height: '100vh', display: 'flex', flexDirection: 'column', background: 'var(--bg)', fontFamily: 'var(--font)', color: 'var(--text)' }}>
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minHeight: 0, minWidth: 0, overflow: 'auto', padding: '20px', paddingBottom: 60 }}>
