...
BEGIN
	...
	begin
		...
		declare @journalNo nvarchar(100) = json_value(@LineData, '$.journalNo')
		...
		where jl.Account = @param1
		  ...
		  and (isnull(@journalNo, '') = '' or jl.JournalNo LIKE '%' + @journalNo + '%')
		  and (
	...
	begin
		...
		declare @jeJournalNo nvarchar(100) = json_value(@LineData, '$.journalNo')
        ...
        begin
            ...
            ';
            exec sp_executesql @DynSQL, N'@jeFrom date, @jeTo date, @jeJournalNo nvarchar(100)', @jeFrom = @jeFrom, @jeTo = @jeTo, @jeJournalNo = @jeJournalNo;
        end
        else
        begin
            select 
    ...
    BEGIN
        ...
        SELECT r.ReportID, r.ReportName, r.[FileName], r.PageGroupID, pg.Label AS PageLabel, r.KeyParam, r.Description, r.IsFastReport, r.CreatedBy, r.CreatedDate
    ...
    BEGIN
        ...
        DECLARE @RptIsFastReport BIT = ISNULL(TRY_CAST(NULLIF(JSON_VALUE(@LineData, '$.IsFastReport'), '') AS BIT), 1);
        ...
        BEGIN
            ...
            SET ReportName = @RptReportName,
                ...
                IsFastReport = @RptIsFastReport
            WHERE ReportID = @RptReportID;
        END
        ELSE
        BEGIN
            INSERT INTO [PLS].[ReportsMaster] (ReportName, [FileName], PageGroupID, KeyParam, Description, IsFastReport, CreatedBy)
            VALUES (@RptReportName, @RptFileName, @RptPageGroupID, @RptKeyParam, @RptDescription, @RptIsFastReport, @User);
